...
def retrieve_all_movies_details(skip: int = 0, limit: int = 100,
...
def delete_movie_by_id(sl_id: int, db: Session = Depends(get_db)):
    ...
    if not details:
        raise HTTPException(status_code=404,
                            detail="No record found to delete")
...
def update_movie_details(sl_id: int, update_param: schema.UpdateMovie,
                         db: Session = Depends(get_db)):
    details = crud.get_movie_by_id(db=db, sl_id=sl_id)
    if not details:
        raise HTTPException(status_code=404,
                            detail="No record found to update")
